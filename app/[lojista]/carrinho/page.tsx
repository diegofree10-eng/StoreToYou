...
import { Menu, X, Grid, Gift, Mail } from "lucide-react";
...
import SecaoClienteEndereco from "./_components/SecaoClienteEndereco";
import ResumoPedidoCard from "./_components/ResumoPedidoCard";
...
export default function CarrinhoIdentidadeVisual() {
  ...
  const isLojaAberta = useMemo(() => {
    ...
    if (lojaAtivaObj.sistema?.isLojaAberta === false || lojaAtivaObj.isLojaAberta === false || lojaAtivaObj.lojaAberta === false || lojaAtivaObj.dadosLoja?.isLojaAberta === false) return false;
    return true;
  ...
  const valorSubTotalComDesconto = useMemo(() => Math.max(0, valorSubtotalProdutos - valorDesconto), [valorSubtotalProdutos, descontoAtivo]);
  ...
  }, [valorSubTotalComDesconto, freteSel?.price, temFrete, freteGratisConfig.atingido]);
  ...
  useEffect(() => {
    async function carregarDadosLojista() {
      ...
      const lojaAtual = dadosLoja || dadosLojaContext;
      const arrayCatsObj = lojaAtual?.categorias || lojaAtual?.dadosLoja?.categorias;
      let listaNomes: string[] = [];

      if (Array.isArray(arrayCatsObj)) {
        arrayCatsObj.forEach((cat: any) => {
          const n = typeof cat === 'string' ? cat : (cat.nome || cat.dsNomeCategoria || cat.title || cat.name);
          if (n && !listaNomes.includes(n)) listaNomes.push(n);
        });
      }

      if (slugLojista && !currentLojistaId) {
        try {
          // Busca por dsSlug na raiz
          let q = query(collection(db, "lojistas"), where("dsSlug", "==", slugLojista), limit(1));
          let querySnapshot = await getDocs(q);

          // Se não achar, busca dentro de dadosLoja.dsSlug
      ...
      if (currentLojistaId) {
        try {
          const catsSnap = await getDocs(collection(db, "lojistas", currentLojistaId, "categorias"));
          if (!catsSnap.empty) {
            catsSnap.docs.forEach(c => {
              const d = c.data();
              const nome = d.nome || d.dsNomeCategoria || d.title || d.name || c.id;
              if (nome && !listaNomes.includes(nome)) listaNomes.push(nome);
            });
          }
        } catch (err) { console.error("Erro categorias:", err); }
      }

      setCategoriasState(listaNomes.map((nome, index) => ({ id: String(index), nome })));
    }
    carregarDadosLojista();
  }, [slugLojista, lojistaId, dadosLoja, dadosLojaContext]);
  ...
  const irParaCategoria = (nomeCat: string) => {
    ...
    router.push(`/${slugLojista}/PagCategoria?cat=${encodeURIComponent(nomeCat)}`);
  };
    ...
    async function calcularTudo() {
      try {
        ...
        if (freteGratisConfig.atingido) {
          // 🎉 Meta atingida: Força a opção de Frete Grátis Promocional
          ...
          // 🔄 Meta NÃO atingida (ou o cliente removeu itens): Restaura as transportadoras normais
          listaFinal = listaFiltrada;

          if (listaFinal.length > 0) {
            // Se havia um frete salvo em cache/backup que não era o gratuito, tenta restaurá-lo
            const freteAnteriorValido = listaFinal.find(f => f.id === freteBackup?.id);
            // Se não tiver backup, seleciona a primeira transportadora disponível
  ...
  useEffect(() => {
    ...
    const lojaAtual = dadosLoja || dadosLojaContext;
    const fontPix = lojaAtual?.pagamentos?.dsChavePix || lojaAtual?.chavePix || lojaAtual?.pix || "";
    ...
    <div style={{ backgroundColor: config.corFundoSite, color: config.corTexto, minHeight: '100vh', fontFamily: 'sans-serif', boxSizing: 'border-box', paddingBottom: '60px', overflowX: 'hidden', position: 'relative' }}>
      ...
      <header className="header-loja" style={{
        ...
        <div className="header-actions-right" style={{ display: 'flex', alignItems: 'center', gap: '18px', justifySelf: 'end' }}>
          ...
          <div className="divisor-desktop" style={{ width: '1px', height: '22px', backgroundColor: '#e2e8f0' }}></div>

          <button className="menu-sanduiche-mobile-btn" onClick={() => setMenuMobileAberto(true)} style={{ background: '#f1f5f9', border: 'none', cursor: 'pointer', color: config.corTexto, display: 'none', alignItems: 'center', gap: '6px', padding: '8px 12px', borderRadius: '8px', fontSize: '13px', fontWeight: 'bold' }}>
      ...
      {/* MODAL / MENU SANDUICHE DE CATEGORIAS MOBILE */}
      {menuMobileAberto && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 3000, display: 'flex' }}>
          <div style={{ width: '280px', backgroundColor: '#fff', height: '100%', padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px', boxSizing: 'border-box', boxShadow: '5px 0 25px rgba(0,0,0,0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #eee', paddingBottom: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 'bold', fontSize: '16px', color: config.corTexto }}>
                <Grid color={config.corPrimaria} size={18} /> Menu de Categorias
              </div>
              <button onClick={() => setMenuMobileAberto(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: config.corTexto }}>
                <X size={24} />
              </button>
            </div>
            <div style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '6px', flex: 1 }}>
              {categoriasState.length > 0 ? (
                categoriasState.map((cat: any) => (
                  <div key={cat.id || cat.nome} onClick={() => irParaCategoria(cat.nome)} style={{ padding: '12px 14px', fontSize: '13px', fontWeight: 'bold', color: config.corTexto, borderRadius: '8px', cursor: 'pointer', backgroundColor: '#f8fafc', textTransform: 'uppercase' }}>
                    {cat.nome}
                  </div>
                ))
              ) : (
                <div style={{ padding: '15px', fontSize: '13px', color: '#64748b', textAlign: 'center' }}>Nenhuma categoria encontrada.</div>
              )}
            </div>
          </div>
...
const summaryColumnStyle: React.CSSProperties = { flex: '1 1 340px', width: '100%', boxSizing: 'border-box' };

const styles: Record<string, React.CSSProperties> = {
  grid: { display: 'flex', gap: '25px', flexWrap: 'wrap', width: '100%', boxSizing: 'border-box', alignItems: 'flex-start' },
  mainColumn: { flex: '2 1 650px', width: '100%', boxSizing: 'border-box' }
};